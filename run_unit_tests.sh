...
run_via_docker() {
    local docker_runner="${ROOT_DIR}/run_unit_tests_docker.sh"

    if [[ "${RUNNING_IN_DOCKER:-0}" == "1" ]]; then
        die "Docker delegation requested while already running inside container. Re-run without --docker."
    fi

    [[ -x "${docker_runner}" ]] || die "Docker runner not found or not executable: ${docker_runner}"

    log "Delegating to Docker runner"
    exec "${docker_runner}" "${FORWARD_ARGS[@]}"
}
...
if ! resolve_prefix; then
    if [[ "${AUTO_DOCKER_ON_MISSING_DEPS}" == "ON" && -x "${ROOT_DIR}/run_unit_tests_docker.sh" ]]; then
        log "Missing host dependencies; auto-falling back to Docker flow"
        run_via_docker
    fi
    print_prefix_hints_and_die
fi
